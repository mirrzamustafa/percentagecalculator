...
export default function CalculatorClient({
  ...
  const inputClass = "w-full border-2 border-slate-400 bg-white p-3 text-xl font-semibold text-slate-900 focus:border-blue-400 focus:ring-4 focus:ring-blue-100 outline-none rounded shadow-sm no-spinner transition-all";
  const labelClass = "mb-2 text-base sm:text-lg font-semibold text-slate-800 ";
  ...
  return (
    ...
    <header className="border-b-3 border-blue-400 bg-white py-4 shadow-md sticky top-0 z-50">
      ...
      <div className="mx-auto flex max-w-5xl items-center justify-between px-4 sm:px-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-lg sm:text-xl font-medium tracking-tight text-slate-900 uppercase leading-none">
      ...
      <section className="rounded-xl border-2 border-slate-300 bg-[#ebedef] p-5 sm:p-10 shadow-lg relative">
        <h2 className="mb-6 text-xl font-medium text-[#1a365d] border-b-2 border-slate-300 pb-2 uppercase">{cardA.title}</h2>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
          ...
          <div className="flex flex-row items-center gap-2 flex-1 min-w-0">
            ...
            <div className="w-[35%] sm:w-32 shrink-0 relative">
              ...
              />
              <span className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-600 font-medium text-sm sm:text-base pointer-events-none">
                %
              </span>
            </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-4 w-full lg:w-auto relative">
            {calcA.hint && (
              <span className="absolute -top-5 left-0 text-[10px] font-medium uppercase text-red-600 leading-none">
                {calcA.hint}
              </span>
            )}
            <button 
              onClick={runCalcA} 
              className="w-full lg:w-48 flex items-center justify-center gap-2 rounded bg-blue-400 py-4 text-lg font-medium uppercase text-white shadow-xl active:scale-95 transition-all"
            >
              {ui.calculate} <ChevronRight size={22} strokeWidth={4} />
            </button>
            <div className="w-full lg:w-48 bg-white border-2 border-blue-400 py-3 text-2xl font-medium text-blue-400 text-center rounded shadow-inner">
              {calcA.result || "---"}
            </div>
          </div>
        </div>
        ...
        <section className="rounded-xl border-2 border-slate-300 bg-[#ebedef] p-6 sm:p-10 shadow-lg relative">
          <h2 className="mb-6 text-xl font-medium text-[#1a365d] border-b-2 border-slate-300 pb-2 uppercase">{cardB.title}</h2>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-6">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4 w-full lg:w-auto relative">
              {calcB.hint && (
                <span className="absolute -top-6 left-0 text-xs font-medium uppercase text-red-600 tracking-tight">
                  {calcB.hint}
                </span>
              )}
              <button onClick={runCalcB} className="w-full lg:w-48 flex items-center justify-center gap-2 rounded bg-blue-400 py-4 text-lg font-medium uppercase text-white shadow-xl active:scale-95 transition-all">
                {ui.calculate} <ChevronRight size={22} strokeWidth={4} />
              </button>
              <div className="w-full lg:w-48 bg-white border-2 border-blue-400 py-3 text-2xl font-medium text-blue-400 text-center rounded shadow-inner flex items-center justify-center gap-1">
                {calcB.result || "---"} <span className="text-blue-400 text-lg font-medium">%</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-xl border-2 border-slate-300 bg-[#ebedef] p-6 sm:p-10 shadow-lg relative">
          <h2 className="mb-6 text-xl font-medium text-[#1a365d] border-b-2 border-slate-300 pb-2 uppercase">{cardD.title}</h2>
          <div className="flex flex-col lg:flex-row lg:items-center gap-6">
            <div className="flex flex-row items-center gap-3 flex-1">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4 w-full lg:w-auto relative">
              ...
              <button onClick={runCalcD} className="w-full lg:w-48 flex items-center justify-center gap-2 rounded bg-blue-400 py-4 text-lg font-medium uppercase text-white shadow-xl transition-all">
                {ui.calculate} <ChevronRight size={22} strokeWidth={4} />
              </button>
              <div className="w-full lg:w-48 bg-white border-2 border-blue-400 py-3 text-2xl font-medium text-blue-400 text-center rounded shadow-inner flex items-center justify-center gap-1">
                {calcD.result || "---"} <span className="text-blue-400 text-lg font-medium">%</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-xl border-2 border-slate-300 bg-[#ebedef] p-5 sm:p-10 shadow-lg relative">
          <h2 className="mb-4 text-xl font-medium text-[#1a365d] border-b-2 border-slate-300 pb-2 uppercase tracking-tight">
            {cardC.title}
          </h2>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-6">
            <div className="flex flex-row items-center gap-3 flex-1 min-w-0">
              ...
              <span className="text-xs sm:text-sm font-semibold text-slate-700 uppercase shrink-0">{cardC.labelChange}</span>
              <div className="w-[35%] sm:w-32 shrink-0 relative">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4 w-full lg:w-auto relative">
              {calcC.hint && <span className="absolute -top-5 left-0 text-[10px] font-medium uppercase text-red-600 leading-none">{calcC.hint}</span>}
              <button onClick={runCalcC} className="w-full lg:w-48 flex items-center justify-center gap-3 rounded bg-blue-400 py-4 text-lg font-medium uppercase tracking-tighter text-white shadow-xl hover:bg-blue-800 active:scale-95 transition-all">
                {ui.calculate} <ChevronRight size={22} strokeWidth={4} />
              </button>
              <div className="w-full lg:w-48 bg-white border-2 border-blue-400 p-3.5 text-2xl font-medium text-blue-400 text-center rounded shadow-inner">
                {calcC.result || "---"}
              </div>
            </div>
          </div>
        ...
        <div className="mt-24 space-y-16 border-t-4 border-slate-300 pt-20 pb-12">
          <div className="max-w-3xl">
            ...
            <p className="text-md font-medium text-slate-900 leading-relaxed italic border-l-8 border-blue-400 pl-6">
              {seo.intro}
            </p>
          </div>
          ...
          <div className="grid gap-12 lg:grid-cols-1">
            <div className="bg-white border-2 border-slate-300 p-8 rounded-lg shadow-sm">
              <h3 className="text-xl font-semibold text-slate-900 mb-6 uppercase tracking-tight">{seo.howTo.title}</h3>
              <p className="text-md text-slate-900 font-medium mb-4">{seo.howTo.content}</p>
              ...
              <p className="text-md text-slate-900 font-medium">{seo.howTo.example}</p>
            </div>

            {/* <div className="bg-white border-2 border-slate-300 p-8 rounded-lg shadow-sm">
              <h3 className="text-2xl font-medium text-slate-900 mb-6 uppercase tracking-tight">{seo.situations.title}</h3>
              <ul className="space-y-4">
                {seo.situations.items.map((item: string, idx: number) => (
                  <li key={idx} className="flex gap-4 text-slate-800 font-bold text-lg items-start">
                    <Zap size={24} className="text-blue-400 shrink-0 mt-1" fill="currentColor" />
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <div className="space-y-8">
            <h3 className="text-2xl font-semibold text-slate-900 text-center capitalize tracking-tight">
              {seo.examples.title}
            </h3>
    ...
    {seo.faq.map((item: any, idx: number) => (
      <div key={idx} className="p-6 sm:p-8 bg-white rounded-xl border-2 border-slate-100 shadow-sm hover:border-blue-200 transition-all">
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-400 text-white text-sm font-medium shadow-md">
            ?
          </span>
          <p className="font-semibold text-slate-900 text-lg sm:text-xl leading-tight pt-1">
            {item.question}
          </p>
        </div>
        {/* Adjusted padding-left to align perfectly with the text above */}
        <p className="text-slate-700 font-medium leading-relaxed pl-14">
          {item.answer}
        </p>
      </div>
    ))}
